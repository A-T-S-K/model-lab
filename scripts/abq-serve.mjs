...
const host=process.env.MODEL_LAB_HOST??'127.0.0.1';
if(!Number.isInteger(port)||port<1024||port>65535)throw Error('PORT must be an integer from 1024 to 65535');
if(!['127.0.0.1','0.0.0.0'].includes(host))throw Error('MODEL_LAB_HOST must be 127.0.0.1 or 0.0.0.0');
...
server.on('error',e=>{console.error(`Model Lab launcher failed: ${e.code??e.message}. Choose a free configured host/PORT; no existing process was stopped.`);process.exitCode=1;});
server.listen(port,host,()=>console.log(`Model Lab prepared exhibit: http://${host}:${port}/?presentation=spatial&kiosk=1`));

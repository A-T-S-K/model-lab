...
export function compareMatchedVariantRuns(baseline: RecordedRun, variant: RecordedRun): MatchedVariantComparison {
  ...
  const initialization = variant.manifest.modelInitialization as unknown as VariantInitializationRecord | undefined;
  if (!initialization || initialization.formatVersion !== 1) reasons.push('variant initialization record is missing');
    ...
    if (initialization.id !== variant.manifest.startingCheckpointId) reasons.push('variant checkpoint does not identify the mapped initialization');
  ...
  return immutableCopy({
    ...
    artifacts: reasons.length ? [] : artifacts,
  });
}

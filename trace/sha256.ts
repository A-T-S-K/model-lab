...
/** Standards-compatible SHA-256 for HTTP origins where SubtleCrypto is unavailable. */
function portableSha256(source: Uint8Array): Uint8Array<ArrayBuffer> {
  ...
  paddingView.setUint32(paddedLength - 8, Math.floor(bitLength / 0x1_0000_0000), false);
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 16; index++) schedule[index] = block.getUint32(index * 4, false);
...
export async function sha256Bytes(
  bytes: Uint8Array,
  source: Sha256Source | undefined = globalThis.crypto,
): Promise<Uint8Array<ArrayBuffer>> {
  if (!source?.subtle) return portableSha256(bytes);
  const input = new Uint8Array(bytes.byteLength);
  input.set(bytes);
  // A present native implementation remains authoritative: rejection must propagate.
  return new Uint8Array(await source.subtle.digest('SHA-256', input));
}

export async function sha256Hex(bytes: Uint8Array, source?: Sha256Source): Promise<string> {
  const digest = await sha256Bytes(bytes, source);
  return Array.from(digest, byte => byte.toString(16).padStart(2, '0')).join('');
}

export async function sha256Id(bytes: Uint8Array, source?: Sha256Source): Promise<string> {
  return `sha256:${await sha256Hex(bytes, source)}`;
}

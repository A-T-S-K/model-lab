...
export const outputTokenName = (id:number, vocabulary:readonly string[]) => vocabulary[id] ?? 'END';
...
export function outputSummary(pair:OutputPair,position:number,labels:[string,string]) {
 ...
 const token=(i:number)=>outputTokenName(i,vocabulary);

...
export function operationConstruction(m:SpatialReadModel,a:Address,element:number,execution?:ForwardProgress): OperationConstruction {
  ...
  if(a.kind==='probabilities'){
    ...
    detailMath=`<div class="construction-softmax">${m.forward.input.map((_,i)=>`<div><strong>key ${i}</strong>${i>a.token?'<p>future · NA</p>':`<p>score ${value(e.scoreInputs?.[i])}</p><p>exp(score − ${value(e.maximum)})</p><p>${value(e.exponentials?.[i])} / ${value(e.denominator)}</p><p>→ P ${value(e.output?.[i])}</p>`}</div>`).join('')}</div>`;
    notes=`Shifted exponentials and denominator are derived from observed scores. Shared sum = ${value(e.denominator)}. Probabilities above are observed; consumer: weighted values.`;
    ...
    notes=`Two distinct normalizations are preserved: embeddingNorm precedes preAttentionNorm before attention projection.`;
    ...
    essentialSummary=`<p>Canonical MLP pipeline: preMlpNorm (RMSNorm ${value(preNorm.observed)}) → mlpUp (8→32 projection ${value(up.observed)}) → ReLU (${value(act.observed)}) → mlpDown (32→8 projection ${value(down.observed)}) → mlpResidual (residual addition ${value(res.observed)}).</p><p>Canonical MLP pipeline: preMlpNorm → mlpUp → ReLU → mlpDown → mlpResidual.</p>`;
    ...
    notes=`Canonical MLP pipeline: preMlpNorm → mlpUp → ReLU → mlpDown → mlpResidual.`;
    ...
    notes=`Raw token scores are unnormalized logits, not probabilities. Softmax converts these into a normalized probability distribution in the next step.`;
  }else{
  ...
  return { purpose, essentialSummary, detailMath, notes };
}

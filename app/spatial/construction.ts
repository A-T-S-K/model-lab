...
export function operationConstruction(m:SpatialReadModel,a:Address,element:number,execution?:ForwardProgress): OperationConstruction {
  ...
  if(a.kind==='probabilities'){
    ...
    plainMeaning="The model compares what this character is looking for (Query) with what earlier characters provide (Key) to calculate relevance scores.";
    ...
    plainMeaning="The model converts raw relevance scores into percentages (probabilities) that add up to 100%. Higher percentages mean the model pays more attention to that character.";
    ...
    plainMeaning="The model uses attention weights to blend information from relevant earlier positions into a combined context vector for this attention head.";
    ...
    plainMeaning="Attention mixes context. The model looks back at earlier characters to determine which ones are relevant to the current position, combining their information and adding it to the saved representation.";

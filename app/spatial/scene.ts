...
export function stationForWorld(f:ForwardModel,kind:string,head=0,layer=0):Station {
  ...
  const composite=f.operations.some(operation=>operation.kind==='mlpCompositeDown'),span=composite?3400:2500;
  const base=850+layer*span,row=220+head*270;
  ...
  const layerX:Record<string,number>={preAttentionNorm:70,attentionOutput:1160,attentionProjection:1300,attentionResidual:1440,preMlpNorm:1580,mlpUp:1720,mlpRelu:1860,mlpLeakyRelu:1860,mlpDown:2000,
    mlpBaseDown:2050,mlpAdapterA:2050,mlpAdapterB:2220,fixedAdapterScale:2390,mlpAdapterScaled:2560,mlpCompositeDown:2740,mlpResidual:2910};
  const branchY:Record<string,number>={mlpBaseDown:410,mlpAdapterA:700,mlpAdapterB:700,fixedAdapterScale:700,mlpAdapterScaled:700,mlpCompositeDown:540};
  if(layerX[kind]!==undefined)return {kind,layer,x:base+layerX[kind],y:branchY[kind]??540,width:kind==='fixedAdapterScale'?135:105,height:165};
  const globalX:Record<string,number>={tokenEmbedding:180,positionEmbedding:180,embeddingSum:360,embeddingNorm:540,logits:850+f.layers*span,probabilities:1030+f.layers*span};
...
export function sceneSvg(f:ForwardModel,selected:Address,key:number,parameter:string|undefined,labels:string[],query=selected.token,comparison?:{before:ForwardModel;after:ForwardModel},learningMarkup?:string,execution?:ForwardProgress,element=0) {
  if(f.descriptor.presentation!=="microgpt-canonical-curated")return repeatedSceneSvg(f,selected,key,parameter,labels,query);
  ...
  const line=(a:Station,b:Station,cls="activation")=>{const edgeHead=a.head??b.head;return `<path data-edge-from="${a.kind}" data-edge-to="${b.kind}" ${edgeHead===undefined?"":`data-edge-head="${edgeHead}"`} class="${cls}" d="M${a.x+a.width} ${a.y+a.height/2} C${a.x+a.width+45} ${a.y+a.height/2} ${b.x-45} ${b.y+b.height/2} ${b.x} ${b.y+b.height/2}"/>`;};
  const qkvInputBranch=(source:Station,target:Station,head:number,branch:1|2)=>{
    const sourceY=source.y+source.height/2+(head===0?-15*branch:15*branch);
    const railY=head===0?245-(branch-1)*30:885+(branch-1)*25;
    const targetX=target.x+target.width/2,targetY=head===0?target.y:target.y+target.height;
    return `<path data-edge-from="${source.kind}" data-edge-to="${target.kind}" data-edge-head="${head}" class="activation" d="M${source.x+source.width} ${sourceY} C${source.x+source.width+25} ${sourceY} ${source.x+source.width+25} ${railY} ${source.x+source.width+45} ${railY} H${targetX} V${targetY}"/>`;
  };
  const field=(s:Station)=>{
    ...
    ${atFrontier?`<rect class="execution-frontier-outline" x="${s.x-7}" y="${s.y-7}" width="${s.width+14}" height="${s.height+14}" rx="8"/>`:""}
  ...
  return `<svg id="spatial-world" class="spatial-world" viewBox="0 0 4500 1300" tabindex="0" role="group" aria-label="Complete connected forward computation; drag to pan, wheel to zoom"><defs><marker id="forward-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0 L7 3.5 L0 7" fill="#62C7E8"/></marker><marker id="reverse-arrow" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto"><path d="M7 0 L0 3.5 L7 7" fill="#E5A0FF"/></marker></defs>
    ...
    ${[0,1].map(h=>{
      ...
      return line(stations[4],q)+qkvInputBranch(stations[4],k,h,1)+qkvInputBranch(stations[4],v,h,2)+`<path data-edge-from="q" data-edge-to="attentionLogits" data-edge-head="${h}" class="activation" d="M${q.x+q.width} ${q.y+85} C${q.x+q.width+25} ${q.y+85} ${q.x+q.width+10} ${q.y-65} ${q.x+q.width+40} ${q.y-65} H${scores.x+55} V${scores.y}"/>`+(key<=query?`<path data-edge-from="k" data-edge-to="attentionLogits" data-edge-head="${h}" data-selected-key-edge="${key}" class="activation" d="M${k.x+k.width} ${k.y+85} H${k.x+k.width+15} V${k.y+k.height+35} H${scores.x+35} V${scores.y+scores.height}"/>`:"")+line(scores,weights)+line(weights,mix)+(key<=query?`<path data-edge-from="v" data-edge-to="headOutput" data-edge-head="${h}" data-selected-value-edge="${key}" class="activation" d="M${v.x+v.width/2} ${v.y+v.height} V${v.y+v.height+70} H${mix.x+55} V${mix.y+mix.height}"/>`:"")+line(mix,stationFor("attentionOutput"));
    }).join("")}
    ...
    <path data-edge-from="embeddingNorm" data-edge-to="attentionResidual" data-edge-type="saved_residual" class="residual-edge" d="M570 480 C570 15 2390 15 2390 480"/><path data-edge-from="attentionResidual" data-edge-to="mlpResidual" data-edge-type="saved_residual" class="residual-edge" d="M2370 480 C2370 105 3520 105 3520 480"/>
...
function repeatedSceneSvg(f:ForwardModel,selected:Address,key:number,parameter:string|undefined,labels:string[],query:number){
  const composite=f.operations.some(operation=>operation.kind==='mlpCompositeDown'),span=composite?3400:2500;
  const canvasWidth=1250+f.layers*span,canvasHeight=Math.max(1500,420+f.heads*270),selectedLayer=selected.layer??0;
  ...
  const stationsForLayer=(layer:number):Station[]=>[
    ...
    ...["attentionOutput","attentionProjection","attentionResidual","preMlpNorm","mlpUp",f.activationKind,
      ...(composite?["mlpBaseDown","mlpAdapterA","mlpAdapterB","fixedAdapterScale","mlpAdapterScaled","mlpCompositeDown"]:["mlpDown"]),"mlpResidual"].map(kind=>stationForWorld(f,kind,0,layer)),
  ];
  ...
  for(let layer=0;layer<f.layers;layer++){
    ...
    const sequence=['attentionOutput','attentionProjection','attentionResidual','preMlpNorm','mlpUp',f.activationKind];for(let i=0;i<sequence.length-1;i++)edges.push(line(stationForWorld(f,sequence[i],0,layer),stationForWorld(f,sequence[i+1],0,layer),sequence[i]==='attentionProjection'?'saved_residual':'activation'));
    if(composite){const activation=stationForWorld(f,f.activationKind,0,layer),baseDown=stationForWorld(f,'mlpBaseDown',0,layer),a=stationForWorld(f,'mlpAdapterA',0,layer),b=stationForWorld(f,'mlpAdapterB',0,layer),scale=stationForWorld(f,'fixedAdapterScale',0,layer),scaled=stationForWorld(f,'mlpAdapterScaled',0,layer),merge=stationForWorld(f,'mlpCompositeDown',0,layer),residual=stationForWorld(f,'mlpResidual',0,layer);edges.push(line(activation,baseDown),line(activation,a),line(a,b),line(b,scaled),line(scale,scaled,'control_state'),line(baseDown,merge),line(scaled,merge),line(merge,residual,'saved_residual'));}
    else edges.push(line(stationForWorld(f,f.activationKind,0,layer),stationForWorld(f,'mlpDown',0,layer)),line(stationForWorld(f,'mlpDown',0,layer),stationForWorld(f,'mlpResidual',0,layer),'saved_residual'));
  ...
  const banks=f.parameterNames.map((name,index)=>{const owner=f.parameterOwners[name]??parameterOwner(name),layer=owner?.layer,x=owner?.layer===undefined?(name==='lm_head'?canvasWidth-420:150+index*150):850+owner.layer*span+250+(index%8)*310,y=canvasHeight-185,m=f.matrix(name),rows=m?.length??0,cols=m?.[0]?.length??0,isSelected=parameter===name,max=Math.max(0,...(m?.flat()??[]).map(Math.abs));
  ...
  return `<svg id="spatial-world" class="spatial-world" viewBox="0 0 ${canvasWidth} ${canvasHeight}" data-world-width="${canvasWidth}" data-world-height="${canvasHeight}" tabindex="0" role="group" aria-label="Topology-composed ${esc(f.descriptor.label)} computation"><defs><marker id="forward-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0 L7 3.5 L0 7" fill="#62C7E8"/></marker><marker id="reverse-arrow" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto"><path d="M7 0 L0 3.5 L7 7" fill="#E5A0FF"/></marker></defs>
    <text class="region-title" x="20" y="270">INPUT · ${f.input.length} POSITIONS</text>${Array.from({length:f.layers},(_,layer)=>{const x=850+layer*span;return `<rect class="region" x="${x}" y="130" width="${span-180}" height="${Math.max(940,f.heads*270+180)}" rx="8"/><text class="region-title" x="${x+15}" y="100">LAYER ${layer} · ${f.heads} HEADS · WIDTH ${f.width*f.heads}${composite?' · COMPOSITE MLP W x + s B(Ax)':''}</text>`;}).join('')}<text class="region-title" x="${850+f.layers*span}" y="270">OUTPUT</text>

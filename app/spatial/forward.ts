...
function operationsFor(topology:MicrogptTopologyConfig):Operation[]{
  ...
  if(!activation||activation.kind==='mlpRelu')return operations;
  return operations.map(operation=>operation.kind==='mlpRelu'
...
export function dependencies(address:Address,extent:number,heads:number,layers=1,activationKind='mlpRelu'):Dependency[]{
...
export function composeForward(source:ForwardSource){
  ...
  const modelOperations=operationsFor(topology),activationKind=topology.activation?.kind??'mlpRelu';
  ...
  const upstream=(address:Address)=>dependencies(address,input.length,heads,layers,activationKind);
  ...
  const explain=(address:Address,element:number)=>{
    ...
    return {address,semanticAddress:semanticAddress(address),semanticId:semanticId(address),definition:operationDefinition,output,zeroed,headWidth:width,heads,artifact:artifact(address),indexValid,observed:indexValid?output?.[element]:undefined,inputs,parameter,parameterName,terms,
      maximum,scoreInputs,exponentials,denominator,meanSquare,normScale,pairs,before,derivative:!output||before===undefined?undefined:(before>0?1:activationKind==='mlpLeakyRelu'?0.01:0),probabilities,points,mixture,
...
export function forwardReadModel(run:RecordedRun,snapshot:ArchivedSnapshot|undefined){
  ...
  return composeForward({topology:{integration:"microgpt-legacy-v1",modelDefinition:`${run.manifest.model.id}:${run.manifest.model.version}`,label:definition.base?'MicroGPT · Leaky ReLU variant':'Canonical MicroGPT',nLayer:Number(a.nLayer),nHead:Number(a.nHead),nEmbd:Number(a.nEmbd),vocabulary:a.vocabulary as readonly string[],

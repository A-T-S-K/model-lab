...
export function forwardReadModel(run: RecordedRun, snapshot: ArchivedSnapshot | undefined) {
  ...
  const explain=(address:Address,element:number)=>{
    ...
    const terms=definition?.family==="linear"&&inputs&&parameter?.[element]?inputs.map((x,j)=>({index:j,input:x,weight:parameter[element][j],product:x*parameter[element][j]})):undefined;
    const scoreInputs=definition?.family==="softmax"?get(address.kind==="probabilities"?"logits":"attentionLogits",address.token,address.head):undefined;
    ...
    const meanSquare=definition?.family==="norm"&&inputs?inputs.reduce((s,x)=>s+x*x,0)/inputs.length:undefined;
    ...
    const probabilities=address.kind==="headOutput"?get("attentionProbabilities",address.token,address.head):undefined;
    ...
    return {address,definition,output,headWidth:width,heads,artifact:artifact(address),indexValid,observed:indexValid?output?.[element]:undefined,inputs,parameter,terms,
      maximum,scoreInputs,exponentials,denominator,meanSquare,normScale,pairs,before,derivative:before===undefined?undefined:Number(before>0),
  ...
  return {input,heads,width,addresses,matrix,artifact,values,upstream,downstream,explain,sourceSnapshotId:compatible?snapshot?.id:undefined};
}

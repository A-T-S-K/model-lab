...
const p=this.playback,available=this.routeChoice==='forward'?this.model?.valid:this.state?.learning?.available;
  ...
  render(m:SpatialReadModel|undefined,state:PresentationState) {
    ...
    return `<div class="spatial-shell"><header class="spatial-header"><div class="world-brand"><strong>MODEL LAB</strong><small>one tiny transformer · one connected computation</small></div><label>Input<input id="document" type="text" maxlength="7" value="${esc(state.document)}" ${state.busy?"disabled":""}></label><button id="predict" ${state.busy||state.execution||!state.ready?"disabled":""}>Predict</button>${state.execution?"":`<button id="step-prediction" ${state.busy||!state.ready?"disabled":""}>Step through prediction</button>`}<button id="spatial-learn" ${state.canLearn?"":"disabled"}>Learn · one update</button><button id="spatial-home">⌂ Home</button><button id="spatial-back" ${!this.history.length?"disabled":""}>← Back</button><button id="spatial-focus">◎ Focus</button><button id="spatial-lens">Q/K lens</button><button id="presentation-toggle">Classic presentation</button><button id="clear-session">Clear session</button><span class="spatial-badge">${state.execution?"LIVE FORWARD · WAVE 2A":"COMPLETED EVIDENCE · WAVE 1D"}</span></header>
      <div class="spatial-status"><span role="status" data-testid="status">${esc(state.status.replace(/ · [a-f0-9-]{36}:.*$/, ""))}</span><span data-testid="spatial-relationship">${m?`${state.execution?'ACTIVE EXECUTION':m.source.relationship} · captured input ${esc(m.source.capturedDocument)}`:"NO SELECTED RUN"}</span></div>${state.error?`<p role="alert">${esc(state.error)}</p>`:""}

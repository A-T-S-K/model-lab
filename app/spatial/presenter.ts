...
export class SpatialPresenter {
  ...
  resetVisitor(){
    this.invalidate(); this.camera.detach(); this.camera.move({...HOME},false);
    this.history=[]; this.boundSelection=undefined; this.boundPin=undefined;
    this.model=undefined; this.state=undefined; this.routeChoice='forward';
    Object.assign(this.selection,{query:4,key:0,head:0,feature:0});
    ...
    this.operatorControls=false; this.construction=false; this.shortStop=-1; this.shortDetour=false;
  }
  private shortRoute(index:number){
    ...
    this.shortStop=index; this.shortDetour=false; this.construction=true; this.lens=false;
  }
  private redraw=()=>{};
  private guideApply=()=>{};
  ...
  invalidate(){this.shortStop=-1;this.shortDetour=false;this.playback.invalidate();this.guided=false;this.detour=false;this.step=0;this.camera.stop();this.pendingBox=undefined;}
  private guide(){
...
const p=this.playback,available=this.routeChoice==='forward'?this.model?.valid:this.state?.learning?.available;
  ...
  render(m:SpatialReadModel|undefined,state:PresentationState) {
    ...
    return `<div data-retention="${esc(JSON.stringify(state.retention))}" class="spatial-shell ${state.attract?"spatial-attract":""} ${state.exhibit&&!state.execution&&!this.operatorControls?"visitor-controls":""}"><header class="spatial-header"><div class="world-brand"><strong>MODEL LAB</strong><small>one tiny transformer · one connected computation</small></div><label>Input<input id="document" type="text" maxlength="7" value="${esc(state.document)}" ${state.busy?"disabled":""}></label><button id="predict" ${state.busy||state.execution||!state.ready?"disabled":""}>Predict</button>${state.execution?"":`<button id="step-prediction" ${state.busy||!state.ready?"disabled":""}>Step through prediction</button><button id="step-learning" ${state.busy||!state.ready?"disabled":""}>Step through learning</button>`}<button id="spatial-learn" ${state.canLearn?"":"disabled"}>Learn · one update</button><button id="spatial-home">⌂ Home</button><button id="spatial-back" ${!this.history.length?"disabled":""}>← Back</button><button id="spatial-focus">◎ Focus</button><button id="spatial-lens">Q/K lens</button><button id="presentation-toggle">Classic presentation</button><button id="clear-session">Clear session</button><span class="spatial-badge">${state.attract?"RECORDED RUN · REPLAY":state.execution?(state.execution.progress?.training?"LIVE TRAINING · WAVE 2B":"LIVE FORWARD · WAVE 2A"):"COMPLETED EVIDENCE · WAVE 1D"}</span></header>
      <div class="spatial-status"><span role="status" data-testid="status">${esc(state.status.replace(/ · [a-f0-9-]{36}:.*$/, ""))}</span>${state.ablationPending?'<button id="cancel-ablation">Cancel head comparison</button>':""}<span data-testid="spatial-relationship">${m?`${state.execution?'ACTIVE EXECUTION':m.source.relationship} · captured input ${esc(m.source.capturedDocument)}`:"NO SELECTED RUN"}</span></div>${state.error?`<p role="alert">${esc(state.error)}</p>`:""}${!m&&state.execution?this.controls():""}${state.attract?`<section class="exhibit-entry"><div><strong>RECORDED RUN · REPLAY</strong><h1>How does a tiny model choose what comes next?</h1><p>Recorded real run. Not live. Start to make a fresh prediction, then follow the numbers through attention.</p></div><button id="exhibit-start" ${state.ready&&!state.busy?'':'disabled'}>Start · explore a real prediction</button></section>`:m&&!state.execution?`<details class="short-guide" ${state.exhibit?'open':''}><summary>Short teaching route · prediction → attention → learning</summary>${this.operatorControls?`<p>${state.retention?.runs??0} retained runs · ${((state.retention?.bytes??0)/1048576).toFixed(1)} MiB / 64 MiB serialized evidence estimate, plus operation headroom; not total process memory.</p>`:""}<nav aria-label="Short teaching route"><button id="operator-controls">${this.operatorControls?"Hide":"Show"} operator controls</button><button id="short-sample">Sample abca · q3 / h0 / k0</button><button data-short-stop="0">Prediction</button><button data-short-stop="1">Q/K scores</button><button data-short-stop="2">Softmax</button><button data-short-stop="3">Value mixture</button><button data-short-stop="4">Residual</button>${this.shortStop>=0?'<button id="short-resume">Resume short route</button>':''}${state.exhibit&&this.operatorControls?'<button id="exhibit-opt-out">Disable idle reset · facilitated session</button>':''}</nav><p>${this.shortDetour?'Exploring a detour. Resume explicitly to return. ':''}${this.shortStop<1?"Teacher-forced prediction · selected position and known target; learning uses all positions.":"Follow the selected operands → calculation → result. Exact values and source remain available."}</p></details>`:''}
...
`:`<section class="spatial-empty"><h1>One model, a complete forward computation</h1><p>Enter a, b or c, then Predict. Explore its actual operations and their sources.</p></section>`}</div>`;
  ...
  bind(m:SpatialReadModel|undefined,changed:()=>void,render:()=>void,phase?:(phase:string)=>void) {
    ...
    on('#short-resume',()=>{this.shortRoute(Math.max(0,this.shortStop));changed();render();});
    on('#short-sample',()=>{
      if(m?.source.capturedDocument!=='abca'){this.state!.error='The sample selection needs an abca prediction. Enter abca and explicitly Predict first.';render();return;}

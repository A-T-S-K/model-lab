  ...
  switch (state) {
    ...
    case 'p1_represent':
      return {
        ...
        plainMeaning: 'The token embedding and position embedding are added, then normalized into the working representation that enters the transformer block.',
        resultConcept: { label: 'Working representation' },
        selectionIntent: { kind: 'embeddingNorm', token: PART_1_TOKEN, key: PART_1_KEY },
        focus: {
          nodes: [
            { kind: 'tokenEmbedding' },
            { kind: 'positionEmbedding' },
    ...
    case 'p1_qkv':
      return {
        ...
        plainMeaning: 'After pre-attention normalization, three learned projections create Query, Key, and Value vectors. Query and Key supply the two sides of attention comparisons; Value carries information that a later weighted mixture can combine.',

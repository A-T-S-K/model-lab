...
test('generation timeline selects retained native occurrences and discloses uncached policy',{skip:!path},async()=>{
  const envelope=JSON.parse(await readFile(path!,'utf8')),store=new EvidenceStore(integrations()),run=await store.admit(envelope),selection={node:'',port:'',phase:'',coordinates:{}},model=composePythiaWorld(run,envelope,selection,false);

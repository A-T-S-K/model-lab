...
import { detailView, number, probabilityView, vectorView } from '../../app/views/evidence.js';
...
test('unavailable display values are explicit and observed zero remains numerical', () => {
  ...
  const artifact: Artifact = { id: 'test', concept: { kind: 'q' }, kind: 'q', shape: [1], axes: ['feature'], dtype: 'float64', values: null, provenance: 'observed', availability: 'budget_exceeded', captureLevel: 'semantic' };
...
test('future attention display states the computation does not exist', () => {
  const html = detailView({ provenance: 'derived', availability: 'not_applicable', q: [], k: [], products: [], sum: null, scale: null, scaled: null, observedLogit: null, probability: null, logits: [], sourceRunId: 'test' });

...
test('M4-B2 mixed historical archive exports, imports inertly, and survives a tampered replacement attempt',{tag:'@m4-b2'},async({page,context,evidenceDir:directory})=>{
  ...
  await importedPage.locator('#open-shared-inspector').click();await importedPage.locator('#shared-run-id').fill(runId);await importedPage.locator('#shared-run-open').click();await importedPage.locator('#shared-world').click();await importedPage.locator('#pythia-invocation').selectOption('generation:2');await importedPage.locator('#pythia-operation').selectOption('generation:2/logits');const output=importedPage.locator('[data-pythia-coordinate="output_index"]');await output.fill('50303');await output.dispatchEvent('change');await expect(importedPage.getByTestId('pythia-selected-output')).toContainText('-3.8919265270233154');await expect(importedPage.getByTestId('pythia-effective-prefix')).toContainText('generation:2');await expect(importedPage.getByTestId('pythia-cache-policy')).toContainText('cache unsupported/unqualified');

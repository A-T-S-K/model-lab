...
test('Wave 1D paced Forward and Learning explanation review',async({page})=>{
 ...
 await page.locator('#spatial-learn').click();await expect(page.getByTestId('spatial-live-step')).toHaveText('1');await page.locator('#explanation-route').selectOption('learning');await page.locator('#explanation-restart').click();await pause();await page.locator('#waypoint-next').click();await page.locator('#learning-inspect-gradient').click();await expect(page.getByTestId('learning-contributions')).toBeVisible();await page.getByTestId('learning-contributions').scrollIntoViewIfNeeded();await pause();

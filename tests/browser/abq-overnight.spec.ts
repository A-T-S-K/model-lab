...
test('Q02 complete public reset across completed, forward, partial, Ready, historical and intervention',async({page})=>{
 ...
 for(const state of ['completed','forward','partial','ready','historical','intervention']){
  await start(page);
  if(state==='completed'){await page.locator('#operator-controls').click();await page.locator('#explanation-follow').uncheck();await page.locator('#explanation-restart').click();await page.locator('#waypoint-next').click();}
...
test('Q04/Q05 short route detour, keyboard, reduced motion and source invalidation',async({page})=>{
 await page.emulateMedia({reducedMotion:'reduce'});await entry(page);await start(page);await page.locator('#short-sample').press('Enter');await page.locator('[data-short-stop="1"]').press('Enter');const run=await page.getByTestId('scene-construction').getAttribute('data-run');await page.locator('#spatial-operation').selectOption('mlpRelu');await page.locator('#spatial-query').selectOption('0');await expect(page.locator('.short-guide')).toContainText('Exploring a detour');await page.locator('#short-resume').click();await expect(page.getByTestId('scene-construction')).toContainText('Attention scores · p3');expect(await page.getByTestId('scene-construction').getAttribute('data-run')).toBe(run);

...
test('Q02 complete public reset across completed, forward, partial, Ready, historical and intervention',async({page})=>{
 ...
 for(const state of ['completed','forward','partial','ready','historical','intervention']){
  ...
  if(state==='intervention'){await page.locator('#short-sample').click();await page.locator('[data-short-stop="3"]').click();await page.locator('#spatial-operation').selectOption('headOutput');await page.locator('#spatial-ablate').click();await expect(page.getByTestId('spatial-intervention')).toBeVisible({timeout:30000});await expect(page.getByTestId('scene-construction')).toHaveCount(0);await expect(page.getByTestId('teaching-step')).toBeInViewport();await page.locator('#close-spatial-lens').click();await expect(page.getByTestId('scene-construction')).toBeVisible();await expect(page.getByTestId('scene-construction')).toContainText('Declared intervention');const box=await page.getByTestId('scene-construction').boundingBox();expect(box).not.toBeNull();expect(box!.height).toBeGreaterThan(100);expect(box!.y+box!.height).toBeLessThanOrEqual(page.viewportSize()!.height);}

...
for(const [width,height] of [[1920,1080],[1280,720]])test(`Q01/Q03 entry and source-bound chain ${width}`,async({page})=>{
 ...
 for(const stop of [1,2,3,4,5]){await page.locator(`[data-short-stop="${stop}"]`).click();await expect(page.locator('.context-lens')).toBeHidden();await expect(page.getByTestId('scene-construction')).toBeVisible();await page.screenshot({path:`${dir}/chain-${stop}-${width}.png`});const box=await page.getByTestId('scene-construction').boundingBox();expect(box!.height).toBeGreaterThan(150);expect(box!.y+box!.height).toBeLessThanOrEqual(height);}
...
test('Q02 complete public reset across completed, forward, partial, Ready, historical and intervention',async({page})=>{
 ...
 for(const state of ['completed','forward','partial','ready','historical','intervention']){
  ...
  if(state==='intervention'){await page.locator('#short-sample').click();await page.locator('#facilitator-attention-detail').click();await page.locator('#spatial-operation').selectOption('headOutput');await page.locator('#spatial-ablate').click();await expect(page.getByTestId('spatial-intervention')).toBeVisible({timeout:30000});await expect(page.getByTestId('scene-construction')).toHaveCount(0);await expect(page.getByTestId('teaching-step')).toBeInViewport();await page.locator('#close-spatial-lens').click();await expect(page.getByTestId('scene-construction')).toBeVisible();await expect(page.getByTestId('scene-construction')).toContainText('Declared intervention');const box=await page.getByTestId('scene-construction').boundingBox();expect(box).not.toBeNull();expect(box!.height).toBeGreaterThan(100);expect(box!.y+box!.height).toBeLessThanOrEqual(page.viewportSize()!.height);}
  const acceptCount=await page.evaluate(()=>(window as any).abq.commands.filter((c:any)=>c.command==='acceptTraining').length);
  await page.locator('#clear-session').click();await expect(page.locator('#exhibit-start')).toBeEnabled();await page.waitForTimeout(250);await expect(page.locator('#spatial-world')).toHaveAttribute('viewBox','0 0 4500 1700');await expect(page.locator('#execution-controls')).toHaveCount(0);await expect(page.locator('.context-lens')).toBeHidden();await expect(page.locator('#spatial-back')).toBeDisabled();await expect(page.locator('#explanation-follow')).toBeChecked();

...
async function ready(page:Page){await page.locator('#step-learning').click();await page.locator('#execution-pin').click();await expect(page.locator('#execution-continue')).toBeEnabled({timeout:60000});await page.locator('#execution-continue').click();await expect(page.locator('#execution-controls')).toHaveAttribute('data-training-phase','ready',{timeout:60000});}
for(const [width,height] of [[1920,1080],[1280,720]])test(`Q01/Q03 entry and source-bound chain ${width}`,async({page})=>{
 ...
 for(const stop of [1,2,3,4,5]){await page.locator(`[data-short-stop="${stop}"]`).click();await expect(page.locator('.context-lens')).toHaveCount(0);await expect(page.getByTestId('scene-construction')).toBeVisible();await page.screenshot({path:`${dir}/chain-${stop}-${width}.png`});const box=await page.getByTestId('scene-construction').boundingBox();expect(box!.height).toBeGreaterThan(150);expect(box!.y+box!.height).toBeLessThanOrEqual(height);}
 await page.locator('[data-short-stop="2"]').click();await expect(page.getByTestId('scene-construction')).toContainText('Saved embeddingNorm');await page.locator('#facilitator-attention-detail').click();await expect(page.getByTestId('scene-construction')).toContainText('All 4 original component products');
...
test('Q02 complete public reset across completed, forward, partial, Ready, historical and intervention',async({page})=>{
 ...
 for(const state of ['completed','forward','partial','ready','historical','intervention']){
  await start(page);
  await page.locator('#operator-controls').click();
  if(state==='completed'){await page.locator('#explanation-follow').uncheck();await page.locator('#explanation-restart').click();await page.locator('#waypoint-next').click();}
  if(state==='forward'){await page.locator('#step-prediction').click();await page.locator('#execution-continue').click();}
  if(state==='partial'){await page.locator('#step-learning').click();await page.locator('#execution-pin').click();await expect(page.locator('#execution-continue')).toBeEnabled({timeout:60000});}
  if(['ready','historical'].includes(state)){await ready(page);if(state==='historical'){await page.locator('#execution-accept').click();await page.locator('#spatial-experiment').selectOption({index:1});await page.locator('[data-learning-phase="before"]').click();}}
  if(state==='intervention'){await page.locator('#short-sample').click();await page.locator('#facilitator-attention-detail').click();await page.locator('#spatial-operation').selectOption('headOutput');await page.locator('#spatial-ablate').click();await expect(page.getByTestId('spatial-intervention')).toBeVisible({timeout:30000});await expect(page.locator('.context-lens')).toHaveCount(0);await expect(page.getByTestId('scene-construction')).toBeVisible();await expect(page.getByTestId('scene-construction')).toContainText('Declared intervention');const box=await page.getByTestId('scene-construction').boundingBox();expect(box).not.toBeNull();expect(box!.height).toBeGreaterThan(100);expect(box!.y+box!.height).toBeLessThanOrEqual(page.viewportSize()!.height);}
  const acceptCount=await page.evaluate(()=>(window as any).abq.commands.filter((c:any)=>c.command==='acceptTraining').length);
  await page.locator('#clear-session').click();await expect(page.locator('#exhibit-start')).toBeEnabled();await page.waitForTimeout(250);await expect(page.locator('#spatial-world')).toHaveAttribute('viewBox','0 0 4500 1300');await expect(page.locator('#execution-controls')).toHaveCount(0);await expect(page.locator('.context-lens')).toBeHidden();await expect(page.locator('#spatial-back')).toBeDisabled();await expect(page.locator('#explanation-follow')).toBeChecked();
...
test('Q04/Q05 short route detour, keyboard, reduced motion and source invalidation',async({page})=>{
 await page.emulateMedia({reducedMotion:'reduce'});await entry(page);await start(page);
 await page.locator('#operator-controls').click();
 await page.locator('#short-sample').press('Enter');await page.locator('[data-short-stop="1"]').press('Enter');const run=await page.getByTestId('scene-construction').getAttribute('data-run');await page.locator('#spatial-operation').selectOption('mlpRelu');await page.locator('#spatial-query').selectOption('0');await expect(page.locator('.short-guide')).toContainText('Exploring a detour');await page.locator('#short-resume').click();await expect(page.getByTestId('scene-construction')).toContainText('Pre-attention RMSNorm · L0 · p3');expect(await page.getByTestId('scene-construction').getAttribute('data-run')).toBe(run);
 await page.locator('#document').fill('a');await page.locator('#predict').click();await expect(page.getByTestId('status')).toContainText('Live prediction complete');expect(await page.getByTestId('scene-construction').getAttribute('data-run')).not.toBe(run);await expect(page.getByTestId('scene-construction')).not.toContainText('NaN');await page.locator('#short-sample').click();await expect(page.locator('.short-guide')).toContainText('Enter abca and explicitly Predict first');await expect(page.locator('#document')).toHaveValue('a');
});
test('Q08 test-only serialized-estimate fixture refuses new work visibly and Clear recovers',async({page})=>{
 await page.addInitScript(()=>{const original=TextEncoder.prototype.encode;let predictCalls=0,budgetExceeded=false;TextEncoder.prototype.encode=function(input?:string){const bytes=original.call(this,input);if(input?.includes('"directRuns":[]')){budgetExceeded=false;predictCalls=0;}if((window as any).abqBudgetFixture&&input?.includes('directRuns')&&input.includes('"snapshots":[')){predictCalls++;if(predictCalls>=2){(window as any).abqBudgetFixture=false;budgetExceeded=true;}return bytes;}if(budgetExceeded&&input?.includes('directRuns')&&input.includes('"snapshots":[')){Object.defineProperty(bytes,'length',{value:24*1024*1024});Object.defineProperty(bytes,'byteLength',{value:24*1024*1024});}return bytes;};});
 await audit(page);await entry(page);await start(page);
 await page.locator('#operator-controls').click();
 await page.evaluate(()=>(window as any).abqBudgetFixture=true);await page.locator('#predict').click();await expect(page.getByTestId('status')).toContainText('Live prediction complete');
 expect(await page.evaluate(()=>(window as any).abqBudgetFixture)).toBe(false);const count=await page.evaluate(()=>(window as any).abq.commands.length);await page.locator('#step-learning').click();await expect(page.getByRole('alert')).toContainText('Retention capacity');expect(await page.evaluate(()=>(window as any).abq.commands.length)).toBe(count);
 await page.locator('#spatial-operation').selectOption('headOutput');await page.locator('#spatial-ablate').click();await expect(page.getByRole('alert')).toContainText('Retention capacity');expect(await page.evaluate(()=>(window as any).abq.commands.length)).toBe(count);
 ...
 await writeFile(`${dir}/budget-fixture.json`,JSON.stringify({method:'Test-only override of one main-thread JSON serialization manifest length to production 24 MiB threshold; no production policy or numerical bytes changed.',clearRecovered:true,refusedCommands:0},null,2));
});
test('Q02 facilitated opt-out disables expiry while public reset retains the spatial exhibit entry',async({page})=>{

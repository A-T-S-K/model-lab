...
const dir=process.env.STOP_EVIDENCE_DIR??'test-results/learning-stop-fix/focused-http';
...
async function capture(page:Page,name:string){for(const [width,height] of [[1920,1080],[1280,720]]){await page.setViewportSize({width,height});await page.waitForTimeout(500);await page.screenshot({path:`${dir}/${name}-${width}.png`});}await page.setViewportSize({width:1920,height:1080});}
async function state(page:Page){return page.evaluate(()=>(window as any).stopAudit.progress.at(-1));}
test('S01/S02/S05 untimed initial request → actual partial → next → Ready → Accept; separate discard',async({page})=>{
 ...
 const initial=await state(page);await capture(page,'initial');await page.waitForTimeout(2000);
 ...
 await capture(page,'partial');await page.waitForTimeout(3000);expect((await state(page)).sequence).toBe(partial.sequence);
 await page.locator('#execution-pin').click();await stopped(page);const next=await state(page);expect(next.training.final||next.training.contributions.at(-1).ordinal>e.ordinal).toBe(true);await capture(page,'next');await page.waitForTimeout(2000);
 await page.locator('#execution-continue').click();await expect(page.locator('#execution-controls')).toHaveAttribute('data-training-phase','ready',{timeout:60000});await capture(page,'ready');await page.waitForTimeout(2000);
...
test('S03 no matching embedding row stops before proposals and explains unavailable future',async({page})=>{
...
for(const document of ['a','abcabca'])test('S02 HTTP two accepted updates equal ordinary Learn, including non-embedding prior moments: '+document,async({page,browser,baseURL})=>{
...
test('S04 HTTP delayed acknowledgements: untimed Pause, exploration, pin replacement, input and presentation cancellation',async({page})=>{
